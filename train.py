...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ###################################### Load model ##############################################
    model = UNet(cout=1)
    model(torch.randn([2, 3, args.input_height, args.input_width]))
...
def train(model, args, epochs=10, experiment_name="MI", lr=0.0001, root=".", device=None,
    ...
    should_log = should_write
    if should_log:
        print(f"Training {experiment_name}")
        writer = SummaryWriter("logs/" + datetime.now().strftime("%Y%m%d-%H%M%S"))

    run_id = f"{dt.now().strftime('%h-%d_%H-%M')}-node_bs{args.bs}-tep{epochs}-lr{lr}-wd{args.wd}-{str(uuid.uuid4())[:6]}"
    name = f"{experiment_name}_{run_id}"
    checkpoint_folder = os.path.join(f"checkpoints_{dt.now().strftime('%h-%d_%H-%M')}-{str(uuid.uuid4())[:6]}")
    if should_log:
    ...
    train_loader = DepthDataLoader(args.distributed, args.batch_size, args.num_threads, 'train', args.dataset, args.input_height, args.input_width,
                                   args.filenames_file, args.do_kb_crop, args.do_random_rotate, args.data_path, args.gt_path).data
    test_loader = DepthDataLoader(args.distributed, args.batch_size, args.num_threads, 'online_eval', args.dataset, args.input_height,
                                  args.input_width, args.filenames_file_eval, args.do_kb_crop, args.do_random_rotate, args.data_path, args.gt_path).data
    ...
    cumulated_loss = 10
    # Start from 1 to epochs
    for epoch in range(args.epoch+1, epochs+1):
        ################################# Train loop ##########################################################
        for i, batch in tqdm(enumerate(train_loader), desc=f"Epoch: {epoch}/{epochs}. Loop: Train",
            ...
            ###### Sync across process ######
            if args.distributed:
                # print(f"Cur rank:{args.rank}, step{i}, barrier")
                dist.barrier()
            ...
            l_dense = silog_loss(pred, depth, mask=mask.to(torch.bool), interpolate=False)
            ...
            if should_write and epoch >= args.online_eval_after and step % args.validate_every == 0:
                ...
                if should_log:
                    ...
                    model_io.save_checkpoint(model, optimizer, scheduler, epoch, f"{experiment_name}_{run_id}_latest.pt", os.path.join(root, checkpoint_folder), args.save_weights_only)

                if metrics['abs_rel'] < best_loss and should_write:
                    model_io.save_checkpoint(model, optimizer, scheduler, epoch, f"{experiment_name}_{run_id}_best.pt", os.path.join(root, checkpoint_folder), args.save_weights_only)
        ...
        if should_write:
            model_io.save_checkpoint(model, optimizer, scheduler, epoch, f"{experiment_name}_{epoch}.pt",
                                     os.path.join(root, checkpoint_folder), args.save_weights_only)
...
def validate(args, model, test_loader, criterion_ueff, epoch, epochs, device='cpu'):
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader, desc=f"Epoch: {epoch}/{epochs}. Loop: Validation") if is_rank_zero(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_weights_only", default=False, action="store_true", help="Only save weights if set")
