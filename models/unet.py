...
class Encoder(nn.Module):
    def __init__(self, basemodel_name='tf_efficientnet_b5_ap', pretrained=True):
        ...
        print(f'Finished loading base model {basemodel_name} ' + "pretrained" if pretrained else "not pretrained")
...
class Decoder(nn.Module):
    ...
    def forward(self, x, features):
        ...
        out1 = self.out_block(up4)
        out2 = F.interpolate(out1, scale_factor=2, mode='bilinear', align_corners=True)
        return out2
...
class WeightedDecoderBlock(nn.Module):
    ...
    def forward(self, x, feature):
        ...
        up1 = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=True)
        up2 = torch.cat([up1, feature], 1)
        c1 = self.conv1(up2)
...
class PatchwiseAttention(nn.Module):
    def __init__(self, feature_dim=128, embedding_dim=128, patch_size=[16, 16], num_heads=4, dim_feedforward=1024):
        ...
        encoder_layers = nn.TransformerEncoderLayer(embedding_dim, num_heads, dim_feedforward, activation="gelu")
    ...
    def forward(self, feature):
        ...
        map = self.transformer_encoder(embeddings)
        map = map.permute(1, 2, 0)
        map = map.reshape(b, c, p_h, p_w)
        map = F.interpolate(map, size=[feature.shape[2], feature.shape[3]], mode="area")  # weight the original feature by attention map of patch-size wise
        return map


if __name__ == '__main__':
    x = torch.rand(2, 3, 416, 544)
